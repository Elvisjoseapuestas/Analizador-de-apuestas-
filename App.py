...
for i in range(5):
    ...
    # Aquí es donde tú ingresarías los datos (o donde conectaremos el scraper)
    goles_l = col1.number_input(f"Promedio Goles Local {i+1}", 0.0, 5.0, 1.5)
    goles_v = col2.number_input(f"Promedio Goles Visita {i+1}", 0.0, 5.0, 1.0)
    rating = col1.slider(f"Rating Delantero {i+1}", 5.0, 10.0, 7.0)
    cuota = col2.number_input(f"Cuota Mercado {i+1}", 1.0, 5.0, 1.6)
    ...
    resultados_finales.append({"Partido": f"P{i+1}", "Recomendación": ", ".join(apuestas)})
